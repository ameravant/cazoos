...
describe Event do
  ...
  it "should validate the associated offering before saving" do
    ...
    @event.errors.on(:offering_id).should == 'does not exist'
  end
  ...
  describe "association" do
    it "should have many price options" do
      ...
      price_opt = Factory(:random_event_price_option, :event_id => event.id)
      price_opt2 = Factory(:random_event_price_option, :event_id => event.id)
      price_opt3 = Factory(:random_event_price_option, :event_id => event.id + 1)
      event.event_price_options.should == [price_opt, price_opt2]
      event.event_price_options.should_not include price_opt3      
    end
  end
  describe "scopes" do
    before(:each) do
